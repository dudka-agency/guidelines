...
* *core* - базовые стили (reset, переменные, вспомогательные функции, миксины). Файл fonts должен содержать стили для подключения нестандартных шрифтов используемых на сайте
* *layouts* - содержит стили для основных блоков сайта  
* *components* - содержит стили для конкретных компонентов используемых на сайте (небольшие переиспользуемые блоки) 
* *modules* - содержит полезные наборы стилей для повторного использования  
* *pages* - под каждую страницу создается отдельный файл, который содержит стили только для конкретной страницы. Каждая страница в самом низу должна содержать стили для медизапросов.
* *vendors* - помещаем сюда файлы сторонних библиотек (fontawesome, slick-slider)
